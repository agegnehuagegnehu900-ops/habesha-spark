...
const VideoCard = ({ video }: VideoCardProps) => {
  ...
  return (
    <div
      className="relative flex h-full w-full snap-start items-end"
      ...
      <div className="absolute bottom-24 right-3 flex flex-col items-center gap-5 z-10">
        ...
        <button onClick={handleLike} className="flex flex-col items-center gap-1">
          <motion.div animate={liked ? { scale: [1, 1.4, 1] } : {}} transition={{ duration: 0.3 }}>
            <Heart
              className={`h-7 w-7 ${liked ? "fill-accent text-accent" : "text-foreground"}`}
            />
          </motion.div>
        ...
        <button onClick={() => setSaved(!saved)} className="flex flex-col items-center gap-1">
          <Bookmark
            className={`h-7 w-7 ${saved ? "fill-secondary text-secondary" : "text-foreground"}`}
          />
          <span className="text-xs font-semibold text-foreground">አስቀምጥ</span>
        </button>
        ...
        <div className="mt-1">
          <motion.div
            ...
            className="h-10 w-10 rounded-full border-2 border-muted gradient-ethiopia flex items-center justify-center"
          >
