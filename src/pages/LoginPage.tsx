  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5, type: "spring" }} className="mb-8 text-center">
        ...
        <AnimatePresence mode="wait">
          {step === "phone" ? (
            <motion.div key="phone" initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="w-full max-w-sm space-y-5">
              ...
              <button onClick={handleSendOTP} disabled={phone.length < 9 || loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3.5 text-sm font-bold text-primary-foreground transition-all hover:opacity-90 disabled:opacity-40 glow-green">
              ...
              <div className="flex items-center gap-2 rounded-lg bg-muted/50 p-3">
                ...
                <p className="text-xs text-muted-foreground">{t("secureViaSMS")}</p>
              </div>
            </motion.div>
          ) : (
            <motion.div key="otp" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 20, opacity: 0 }} className="w-full max-w-sm space-y-5">
              <div className="text-center">
                <p className="text-sm text-muted-foreground">{t("codeSentTo")} +251{phone}</p>
              </div>
              ...
              <button onClick={handleVerifyOTP} disabled={otp.length < 6 || loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3.5 text-sm font-bold text-primary-foreground transition-all hover:opacity-90 disabled:opacity-40 glow-green">
              ...
              <button onClick={() => { setStep("phone"); setOtp(""); }} className="w-full text-center text-sm text-muted-foreground hover:text-foreground transition-colors">{t("goBack")}</button>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
